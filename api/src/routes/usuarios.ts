...
// ── POST /usuarios — cadastro de administrador ────────────────────────────
...
// ── GET /usuarios — listagem (protegida) ──────────────────────────────────
...
router.post("/recuperar-senha", async (req, res) => {
  ...
  try {
    await transporter.sendMail({
      from: "Corte Fino <barbearia@cortefino.com>",
      to: email,
      subject: "Recuperação de senha — Corte Fino",
      html: `
